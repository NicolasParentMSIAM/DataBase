...
/* R2- Give the Name and age of the customers that have spend more that 200€ in one skipass*/
...
/* R4- Give the names of the customers that have already followed a ski class and have more than 50 years old*/
...
/* R5- Whats the mean price of the ski pass by age */
select avg(s.SPPrice), c.Age from SkiPass s
join Customer c on c.CustomerName = s.CustomerName
group by c.Age


/*R6- Give the names of customers that have use a building but never bought a ski pass */
select CustomerName from SkiPass s
where CustomerName not in(select CustomerName from GoBuilding)


/*R7- Name of the pistes who are deserved by mechanical lift of capacity less than 50
...
/*R8- Give the customer name that have taken all type of transport  */
